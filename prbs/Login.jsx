...
function Login({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F5F0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'DM Sans, system-ui, sans-serif' }}>
      {/* Left panel */}
      <div style={{ display: 'flex', width: '100%', maxWidth: 960, minHeight: 580, boxShadow: '0 24px 80px rgba(28,24,20,0.13)', borderRadius: 20, overflow: 'hidden' }}>
        {/* Brand panel */}
        <div style={{ flex: '0 0 380px', padding: '56px 48px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
          {/* Blurred campus background */}
          <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(campus.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(4px)', transform: 'scale(1.08)' }} />
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 40 }}>
              <img src="logo.png" alt="AUCA Logo" style={{ width: 56, height: 56, borderRadius: '50%', background: 'white', padding: 3, objectFit: 'contain', flexShrink: 0 }} />
              <div>
        ...
        {/* Form panel */}
        <div style={{ flex: 1, background: '#FFFFFF', padding: '56px 52px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          {step === 'email' ? (
            <div>
              <h2 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 28, fontWeight: 700, color: '#1C1814', margin: '0 0 8px', letterSpacing: '-0.02em' }}>Welcome back</h2>
              ...
              <div style={{ marginTop: 28, padding: 16, background: '#F8F5F0', borderRadius: 10, border: '1px solid #EDE9E2' }}>
                ...
                {[['supervisor@university.ac.rw', 'Supervisor'], ['admin@university.ac.rw', 'Admin'], ['alice@university.ac.rw', 'Student']].map(([em, label]) => (
                  <button key={em} onClick={() => setEmail(em)} style={{ display: 'block', marginTop: 6, background: 'none', border: 'none', padding: 0, color: '#1D5BAF', fontSize: 13, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', textDecoration: 'underline' }}>{em} <span style={{ color: '#7A7069', textDecoration: 'none' }}>({label})</span></button>
                ))}
              </div>
            </div>
          ) : (
            <div>
              ...
              <h2 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 28, fontWeight: 700, color: '#1C1814', margin: '0 0 8px' }}>Check your email</h2>
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 28 }}>
                {otp.map((d, i) => (
                  <input key={i} id={`otp-${i}`} type="text" inputMode="numeric" maxLength={1} value={d}
                    ...
                    style={{ width: 52, height: 60, textAlign: 'center', fontSize: 24, fontWeight: 700, border: '1.5px solid', borderColor: d ? '#1D5BAF' : '#EDE9E2', borderRadius: 10, outline: 'none', fontFamily: 'DM Sans, sans-serif', color: '#1C1814', background: d ? '#E5EDF8' : '#FAFAF8', transition: 'all 0.15s' }}
                  />
                ))}
              </div>
