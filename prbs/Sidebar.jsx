...
const ICONS = {
  ...
  bell:     <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>,
...
function Sidebar({ user, activeTab, setActiveTab, notifCount, onLogout }) {
  const nav = NAV_BY_ROLE[user.role] || [];
  const initials = user.email.slice(0, 2).toUpperCase();

  return (
    <div style={{ width: 240, background: 'linear-gradient(180deg, #0F2755 0%, #0D1F45 100%)', display: 'flex', flexDirection: 'column', height: '100vh', flexShrink: 0 }}>
      {/* Logo */}
      <div style={{ padding: '28px 24px 24px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <img src="logo.png" alt="AUCA" style={{ width: 38, height: 38, borderRadius: '50%', background: 'white', padding: 2, objectFit: 'contain', flexShrink: 0 }} />
          <div>
      ...
      <div style={{ padding: '16px 12px', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <button onClick={onLogout} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', borderRadius: 8, border: 'none', background: 'transparent', color: 'rgba(255,255,255,0.35)', fontSize: 13, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', transition: 'color 0.15s' }}
...
Object.assign(window, { Sidebar, ICONS });
