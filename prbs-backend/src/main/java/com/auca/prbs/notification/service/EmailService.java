...
public class EmailService {
    ...
    public void sendBookingConfirmation(String toEmail, String recipientName,
                                        String project, String date, String time, String meetUrl) {
    ...
    public void sendSupervisorAlert(String supervisorEmail, String supervisorName,
                                    String event, String studentName,
                                    String project, String date, String time) {
        send(supervisorEmail, "Session " + event + " — " + studentName,
                buildSupervisorAlertHtml(supervisorName, event, studentName, project, date, time));
    }
    ...
    private String buildConfirmationHtml(String name, String project, String date, String time, String meetUrl) {
        return """
            <div style="font-family:DM Sans,sans-serif;max-width:520px;margin:auto">
              <div style="background:#0F2755;padding:28px 32px;border-radius:12px 12px 0 0">
              ...
              <div style="background:#ffffff;padding:32px;border:1px solid #EDE9E2;
                ...
                <table style="width:100%%;border-collapse:collapse;margin:20px 0">
                  <tr><td style="padding:10px;background:#F8F5F0;font-size:13px;color:#7A7069;width:40%%">Project</td>
                  ...
                  <tr><td style="padding:10px;background:#F8F5F0;font-size:13px;color:#7A7069">Time</td>
                      <td style="padding:10px;background:#F8F5F0;font-size:14px;font-weight:600;color:#1C1814">%s</td></tr>
                </table>
                <a href="%s" style="display:inline-block;background:#1D5BAF;color:white;
                                    padding:12px 24px;border-radius:8px;text-decoration:none;
                                    font-weight:600;font-size:14px">Join Google Meet →</a>
              </div>
            </div>
            """.formatted(esc(name), esc(project), esc(date), esc(time), escAttr(meetUrl));
    }

    private String buildReminderHtml(String name, String project, String date, String time, String meetUrl) {
        return """
            <div style="font-family:DM Sans,sans-serif;max-width:520px;margin:auto">
              <div style="background:#0F2755;padding:28px 32px;border-radius:12px 12px 0 0">
    ...
    private String buildSupervisorAlertHtml(String supervisorName, String event,
                                            String studentName, String project,
                                            String date, String time) {
        return """
            <div style="font-family:DM Sans,sans-serif;max-width:520px;margin:auto">
              <div style="background:#0F2755;padding:28px 32px;border-radius:12px 12px 0 0">
                <h1 style="color:white;font-size:20px;margin:0">Session %s</h1>
              </div>
              <div style="background:#ffffff;padding:32px;border:1px solid #EDE9E2;
                          border-top:none;border-radius:0 0 12px 12px">
                <p style="color:#1C1814;font-size:15px">Hi %s,</p>
                <p style="color:#7A7069;font-size:14px">
                  %s has %s a review session with you.
                </p>
                <table style="width:100%%;border-collapse:collapse;margin:20px 0">
                  <tr><td style="padding:10px;background:#F8F5F0;font-size:13px;color:#7A7069;width:40%%">Student</td>
                      <td style="padding:10px;background:#F8F5F0;font-size:14px;font-weight:600;color:#1C1814">%s</td></tr>
                  <tr><td style="padding:10px;font-size:13px;color:#7A7069">Project</td>
                      <td style="padding:10px;font-size:14px;font-weight:600;color:#1C1814">%s</td></tr>
                  <tr><td style="padding:10px;background:#F8F5F0;font-size:13px;color:#7A7069">Date</td>
                      <td style="padding:10px;background:#F8F5F0;font-size:14px;font-weight:600;color:#1C1814">%s</td></tr>
                  <tr><td style="padding:10px;font-size:13px;color:#7A7069">Time</td>
                      <td style="padding:10px;font-size:14px;font-weight:600;color:#1C1814">%s</td></tr>
                </table>
              </div>
            </div>
            """.formatted(esc(event), esc(supervisorName), esc(studentName), esc(event),
                          esc(studentName), esc(project), esc(date), esc(time));
    }
    ...
    private static String escAttr(String url) {
        ...
        boolean allowed = lower.startsWith("https://") || lower.startsWith("http://") || lower.startsWith("mailto:");
