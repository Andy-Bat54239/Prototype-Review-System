...
function AdminPanel({ activeTab }) {
  const [users, setUsers] = useAdminState(() =>
    MOCK_USERS.map(u => ({ ...u, otpReset: false }))
  );
  const [settings, setSettings] = useAdminState({ ...MOCK_SETTINGS_INIT });
  ...
  if (activeTab === 'settings') {
    return (
      <div style={{ maxWidth: 580 }}>
        ...
        <div style={{ background: 'white', borderRadius: 14, padding: 32, border: '1px solid #EDE9E2', boxShadow: '0 2px 8px rgba(28,24,20,0.05)' }}>
          {[
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {users.map((user, idx) => {
              ...
              return (
                <tr key={user.id} style={{ borderBottom: idx < users.length-1 ? '1px solid #F8F5F0' : 'none', transition: 'background 0.1s' }}
