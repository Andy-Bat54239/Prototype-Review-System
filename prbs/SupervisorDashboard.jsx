...
const { useState: useSuperState, useEffect: useSuperEffect } = React;

function StatusBadge({ status }) {
  const map = { confirmed: ['#E5EDF8','#1D5BAF','Confirmed'], completed: ['#E8F4E8','#2E7D32','Completed'], 'no-show': ['#FDE8E8','#C62828','No Show'] };
...
function StudentDetailPanel({ booking, onClose, onMark }) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(13,31,69,0.35)', zIndex: 50, transition: 'opacity 0.2s' }} />
      {/* Drawer */}
      <div style={{ position: 'fixed', top: 0, right: 0, height: '100vh', width: 380, background: 'white', zIndex: 51, boxShadow: '-8px 0 40px rgba(13,31,69,0.15)', display: 'flex', flexDirection: 'column', animation: 'slideIn 0.22s ease' }}>
...
function SessionCard({ booking, onMark, onClick }) {
  return (
    <div onClick={onClick} style={{ background: 'white', borderRadius: 12, padding: '18px 20px', border: '1px solid #EDE9E2', display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 1px 6px rgba(28,24,20,0.05)', transition: 'all 0.15s', cursor: 'pointer' }}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ color: '#7A7069', fontSize: 13, display: 'flex', gap: 16 }}>
...
function AnalyticsTab({ bookings }) {
  if (typeof Recharts === 'undefined') {
    return <div style={{ padding: 32, color: '#B8AFA2', fontSize: 15 }}>Loading charts…</div>;
  }
  const { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
          PieChart, Pie, Cell, LineChart, Line } = Recharts;
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'Total Sessions', value: total,    sub: 'all time',  color: '#1D5BAF', bg: '#E5EDF8' },
          { label: 'Completed',      value: completed, sub: 'reviewed', color: '#2E7D32', bg: '#E8F4E8' },
          { label: 'No Shows',       value: noShows,  sub: 'missed',    color: '#C62828', bg: '#FDE8E8' },
          { label: 'Booking Rate',   value: `${rate}%`, sub: 'attendance', color: '#1C1814', bg: '#F0F0EE' },
        ].map(({ label, value, sub, color, bg }) => (
          <div key={label} style={{ background: 'white', borderRadius: 14, padding: '20px 22px', border: '1px solid #EDE9E2', boxShadow: '0 2px 8px rgba(28,24,20,0.05)' }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: color, marginBottom: 14 }} />
            <div style={{ fontSize: 30, fontWeight: 800, color: '#1C1814', fontFamily: 'Playfair Display, serif', marginBottom: 3 }}>{value}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 20 }}>
        ...
        {/* Bar chart */}
        <div style={{ background: 'white', borderRadius: 14, padding: '24px 24px 16px', border: '1px solid #EDE9E2', boxShadow: '0 2px 8px rgba(28,24,20,0.05)' }}>
        ...
        {/* Pie / donut chart */}
        <div style={{ background: 'white', borderRadius: 14, padding: '24px 24px 16px', border: '1px solid #EDE9E2', boxShadow: '0 2px 8px rgba(28,24,20,0.05)', width: 280 }}>
          ...
          <PieChart width={232} height={180}>
            <Pie data={pieData} cx={116} cy={86} innerRadius={52} outerRadius={80} paddingAngle={3} dataKey="value">
...
function AvailabilityTab({ availability, setAvailability }) {
  const [form, setForm] = useSuperState({ date: '', start: '09:00', end: '11:00', duration: '15', meetUrl: '' });
  const setF = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const [added, setAdded] = useSuperState(false);
        ...
        <div style={{ background: 'white', borderRadius: 14, padding: 24, border: '1px solid #EDE9E2', boxShadow: '0 2px 8px rgba(28,24,20,0.05)' }}>
          ...
          {availability.map(a => (
            <div key={a.date} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #F8F5F0' }}>
              <div><div style={{ fontSize: 13.5, fontWeight: 600, color: '#1C1814' }}>{new Date(a.date+'T00:00:00').toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</div><div style={{ fontSize: 12, color: '#B8AFA2' }}>{a.start}–{a.end} · {a.duration}min slots</div></div>
...
function SupervisorDashboard({ activeTab, setActiveTab, bookings, setBookings, availability, setAvailability }) {
  const [selectedDate, setSelectedDate] = useSuperState('2026-04-25');
  const [notif, setNotif] = useSuperState(true);
  const [selectedStudent, setSelectedStudent] = useSuperState(null);
  ...
  if (activeTab === 'analytics') return (
    <div><AnalyticsTab bookings={bookings} /></div>
  );
  if (activeTab === 'availability') return (
    <div><h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: 26, color: '#1C1814', margin: '0 0 24px' }}>Manage Availability</h2><AvailabilityTab availability={availability} setAvailability={setAvailability} /></div>
  );
    ...
    <div>
      {notif && (
        <div style={{ background: 'white', border: '1px solid #EDE9E2', borderLeft: '4px solid #1D5BAF', borderRadius: 10, padding: '14px 18px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 2px 8px rgba(28,24,20,0.06)' }}>
          <div><svg width="22" height="22" fill="none" viewBox="0 0 24 24"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" stroke="#1D5BAF" strokeWidth="1.8" strokeLinecap="round"/></svg></div>
          <div style={{ flex: 1 }}><div style={{ fontWeight: 700, color: '#1C1814', fontSize: 14 }}>Session in 5 minutes: Alice Uwase – Group 3</div><div style={{ color: '#7A7069', fontSize: 13, marginTop: 2 }}>AI Crop Disease Monitor · Apr 25 at 9:00 AM</div></div>
          <button onClick={() => setNotif(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B8AFA2', fontSize: 18, padding: 4 }}>✕</button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {dayBookings.length === 0
          ...
          : dayBookings.map(b => <SessionCard key={b.id} booking={b} onMark={onMark} onClick={() => setSelectedStudent(b)} />)
        }
      </div>
      <StudentDetailPanel booking={selectedStudent} onClose={() => setSelectedStudent(null)} onMark={onMark} />
    </div>
  );
}

Object.assign(window, { SupervisorDashboard });
